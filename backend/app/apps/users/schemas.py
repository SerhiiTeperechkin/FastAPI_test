...
class UserPasswordSchema(BaseModel):
    password: str = Field(examples=["Test_password1"])
    ...
    def validate_password_complexity(cls, value: str) -> str:
        ...
        for error in errors:
            ...
            if error_name == "length":
                ...
                error_messages.append(f"Password must contain at least {error.count} special characters(s)")
        raise ValueError("; ".join(error_messages))


class BaseUserSchema(BaseModel):
    email: EmailStr = Field(description='email of a user', examples=["example@gmail.com"])
    name: Annotated[str,
    StringConstraints(
        pattern=r"^[0-9a-zA-Za-яА-ЯїЇяЯєЄіІґҐ'_. ]+$",
